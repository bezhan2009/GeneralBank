...
pub extern "C" fn transfer_money(conn_ptr: *mut Connection, user_id: i32, acc_num_1: *const libc::c_char, acc_num_2: *const libc::c_char, amount: i32) -> bool {
    ...
    // Создаем заем на объект Connection для вызова transaction()
    let tx_result = conn.transaction();
    if let Ok(mut tx) = tx_result {
        // Создаем заем на объект Connection для вызова savepoint()
        let savepoint_result = tx.savepoint();
        if let Ok(mut savepoint) = savepoint_result {
            // Преобразуем acc_num_1 и acc_num_2 из *const libc::c_char в строки Rust
